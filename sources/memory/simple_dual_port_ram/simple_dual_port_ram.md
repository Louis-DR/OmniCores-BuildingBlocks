...
| `READ_RETURN_TO_X` | integer | `0`, `1`       | `0`     | Return-to-X behavior when `SEQUENTIAL_READ = 1`.<br/>• `0`: Hold last read value.<br/>• `1`: Drive `read_data` to X when `read_enable` is deasserted. |
...
For **read operation**, when `read_enable` is asserted, the data at the memory location specified by `read_address` is driven on `read_data`. The timing depends on the `SEQUENTIAL_READ` parameter:
...
| `read_enable`, `read_address`                 | `read_data` | combinational | Address decoding (if `SEQUENTIAL_READ = 0`).                                |
| `read_enable`, `read_address`                 | `read_data` | sequential    | Address decoding through register (if `SEQUENTIAL_READ = 1`).               |
