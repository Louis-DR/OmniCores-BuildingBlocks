...
![shifter_left](shifter_left.svg)

Shits an input vector `data_in` to the left by a static number of bits specified by the `SHIFT` parameter and pad the right side with the value `PAD_VALUE`.
...
| `SHIFT`     | integer | `≥0`           | `1`     | Number of bit positions to rotate to the left.<br/Rotation by zero doesn't reorder the data.<br/>Rotation by `WIDTH` or more sets all bits of `data_out` to `PAD_VALUE`. |
...
The `shifter_left` module is verified using a SystemVerilog testbench. A top-level testbench, `shifter_left.tb.sv`, instantiates the testcase module, `shifter_left.tc.sv`, once for each parameter set configured.
...
| ------------------ | ------------------------------------------------ | --------------------------------------------------- |
| Design             | [`shifter_left.v`](shifter_left.v)               | Verilog design file.                                |
| Testbench template | [`shifter_left.tb.sv.j2`](shifter_left.tb.sv.j2) | Template of the top-level SystemVerilog testbench.  |
| Testbench          | [`shifter_left.tb.sv`](shifter_left.tb.sv)       | Generated gop-level SystemVerilog testbench.        |
| Testcase           | [`shifter_left.tc.sv`](shifter_left.tc.sv)       | SystemVerilog testcase.                             |
| Waveform script    | [`shifter_left.tb.gtkw`](shifter_left.tb.gtkw)   | Script to load waveforms in GTKWave (assumed).      |
| Symbol descriptor  | [`shifter_left.sss`](shifter_left.sss)           | Symbol descriptor for SiliconSuite-SymbolGenerator. |
| Symbol image       | [`shifter_left.svg`](shifter_left.svg)           | Generated vector image of the symbol.               |
| Datasheet          | [`shifter_left.md`](shifter_left.md)             | Markdown documentation datasheet.                   |
