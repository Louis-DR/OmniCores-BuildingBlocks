...
Otherwise, the divider uses an internal countdown decremented on each positive edge of `clock_in`. When it reaches zero, the output clock is toggled and the countdown is reloaded to the pulse width according to the polarity: `(DIVISION + 1) / 2` for high pulse and `DIVISION / 2` for low pulse.
...
| Module                                                                                   | Path                                                               | Comment                                                   |
| ---------------------------------------------------------------------------------------- | ------------------------------------------------------------------ | --------------------------------------------------------- |
| [`static_clock_multiplier`](../static_clock_multiplier/static_clock_multiplier.md)       | `omnicores-buildingblocks/sources/clock/static_clock_multiplier`   | Static clock multiplier behavioral model.                 |
