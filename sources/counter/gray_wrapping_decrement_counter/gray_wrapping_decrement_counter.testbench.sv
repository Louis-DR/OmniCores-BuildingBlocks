...
// ║ License:     MIT License                                                  ║
// ║ File:        gray_wrapping_decrement_counter.testbench.sv                 ║
// ╟───────────────────────────────────────────────────────────────────────────╢
// ║ Description: Testbench for the gray wrapping decrement counter.           ║
// ║                                                                           ║
// ╚═══════════════════════════════════════════════════════════════════════════╝
...
module gray_wrapping_decrement_counter__testbench ();
...
// Device under test
gray_wrapping_decrement_counter #(
  .RANGE       ( RANGE       ),
  .RESET_VALUE ( RESET_VALUE )
) gray_wrapping_decrement_counter_dut (
  .clock        ( clock        ),
  .resetn       ( resetn       ),
  .decrement    ( decrement    ),
...
initial begin
  // Log waves
  $dumpfile("gray_wrapping_decrement_counter.testbench.vcd");
  $dumpvars(0,gray_wrapping_decrement_counter__testbench);
  $timeformat(-9, 0, " ns", 0);
  ...
  assert (count_binary == COUNT_MAX)
    else $error("[%t] Counter should be at maximum '%0d' after wrapping but is at '%0d'.", $realtime, COUNT_MAX, count_binary);
  check_bit_difference(previous_gray, count_gray);
  assert (count_gray == reference_binary_to_gray(count_binary))
    else $error("[%t] Gray mapping mismatch after wrap up from 0.", $realtime);
  ...
  // First, we are at COUNT_MAX, let's wrap down through the full cycle
  expected_count = COUNT_MAX;
  @(negedge clock);
  decrement = 1;
  timeout_countdown = CHECK_TIMEOUT;
  fork
    ...
    begin
      ...
      assert (count_binary == COUNT_MAX)
        else $error("[%t] Counter should be back to maximum '%0d' after full cycle but is at '%0d'.", $realtime, COUNT_MAX, count_binary);
    end
    // Timeout
    begin
      while (timeout_countdown > 0) begin
  ...
  repeat (RANDOM_CHECK_DURATION) begin
    ...
    previous_gray   = count_gray;
    @(posedge clock);
    expected_count = predict_next_count(previous_binary, decrement);
    @(negedge clock);
    #(1);
    if (decrement) begin
      check_bit_difference(previous_gray, count_gray);
