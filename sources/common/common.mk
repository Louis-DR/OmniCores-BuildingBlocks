...
COMPILE_FLAGS_VERILATOR += +define+SIMULATOR_NO_BOOL
COMPILE_FLAGS_ICARUS    ?=
COMPILE_FLAGS_ICARUS    += -g2012
COMPILE_FLAGS_ICARUS    += -D SIMULATOR_NO_BREAK
COMPILE_FLAGS_ICARUS    += -D SIMULATOR_NO_RANDOMIZE
COMPILE_FLAGS_ICARUS    += -D SIMULATOR_NO_QUEUE_SHUFFLE
COMPILE_FLAGS_ICARUS    += -D SIMULATOR_NO_CONCURRENT_ASSERTION
COMPILE_FLAGS_ICARUS    += -D SIMULATOR_NO_ASSOCIATIVE_MEMORY
COMPILE_FLAGS_MODELSIM  ?=
COMPILE_FLAGS_MODELSIM  += +define+SIMULATOR_NO_BOOL
COMPILE_FLAGS_MODELSIM  += +define+SIMULATOR_NO_REAL_MODULO
...
%.wavedrom.svg: %.wavedrom.json FORCE
	wavedrom-cli -i $< -s $@
	sed -i '' 's/style="stroke:none;fill:white"/style="stroke:none;fill:none"/g' $@
